...
<form action="{{route('cars.update', $car)}}" method="post">
  ...
  <input type="text" name="model" value="{{ isset($car->model) ? $car->model : old('model') }}" placeholder="model">
  <br>
  <br>
  <label>Url Image:</label><br>
  <input type="text" name="imgurl" value="{{ isset($car->imgurl) ? $car->imgurl : old('imgurl') }}" placeholder="imgurl">
  <br>
  <br>
  <label>Year:</label><br>
